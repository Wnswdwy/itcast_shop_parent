...
import cn.itcast.shop.realtime.etl.bean.{CartEntity, CartWideEntity, DimGoodsCatDBEntity, DimGoodsDBEntity, DimOrgDBEntity, DimShopsDBEntity}
...
/**
 * @author yycstart
 * @create 2021-02-02 18:15
 *          购物车的实时ETL
 *   {"addTime":1576479746005,"count":1,"goodsId":"100106",
 *   "guid":"f1eeb1d9-9ee9-4eec-88da-61f87ab0302c","ip":"123.125.71.102","userId":"100208"}
 */
case class CartDataETL(env: StreamExecutionEnvironment) extends MQBaseETL(env){
  /**
  ...
  override def process(): Unit = {
      ...
      val cartWideBeanDS: DataStream[CartWideEntity] = cartBeanDS.map(new RichMapFunction[CartEntity, CartWideEntity] {
        ...
        override def map(cartBean: CartEntity): CartWideEntity = {
          val cartWideBean = CartWideEntity(cartBean)
